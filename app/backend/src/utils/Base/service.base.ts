...
export abstract class BaseService<EntityRef, DTORef, CreateDTORef, UpdateDTORef> {
	...
	constructor(protected readonly repository: Repository<EntityRef>) {}
	...
	public async getById(id: number, req: any): Promise<DTORef> {
		try {
            //TODO: Maybe bug.
			const columnID: any = { id: id };
			const entityRefWhere = columnID as FindOptionsWhere<EntityRef>;

			const entity = await this.repository.findOneBy(entityRefWhere);
		...
		try {
			...
			if (!foundedEntity["active"]) {
				throw new HttpException(
					EntityAlreadyDeactivated(this.entityName, this.isEntityMaleGender),
	...
	public async activateById(id: number, req: any): Promise<UpdateResponse> {
		try {
			...
			if (foundedEntity["active"]) {
				throw new HttpException(
					EntityAlreadyActivated(this.entityName, this.isEntityMaleGender),
					HttpStatus.BAD_REQUEST,
				);
			}
