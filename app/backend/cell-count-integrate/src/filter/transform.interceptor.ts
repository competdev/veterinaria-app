...
import { GenerateDefaultErrorResponse, DefaultResponse, GenerateDefaultResponse, ResponseStatuEnum } from "../utils";
		...
		return next.handle().pipe(
			...
			catchError((err: any) => {
				...
				if (err instanceof HttpException) {
					...
					const errorResponse = GenerateDefaultErrorResponse(err.getStatus(), { ...errorData });
					throw errorResponse;
				}
				...
				const errorResponse = GenerateDefaultErrorResponse(HttpStatus.BAD_REQUEST, { ...errorData });
				throw errorResponse;
			}),
		);
	}
